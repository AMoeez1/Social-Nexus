...
import { MdHomeFilled, MdMenu, MdOutlineExplore, MdOutlineLogin } from "react-icons/md";
...
export default function Sidebar() {
	...
	const sidebar = [
		...
		},
		{
			id: 4,
			title: 'Reels',
			icon: <TbPlayerPlayFilled size={22} />,
			href: '/reels'
		},
		{
			id: 5,
			title: 'Message',
			icon: <SiMessenger size={20} />,
			href: '/direct/inbox'
		},
		{
			id: 12,
	...
	return (
		<div className="flex">
			<div className="w-[19rem] h-screen bg-gray-100 hidden lg:block p-3 border-r-2" style={{ position: 'fixed'}}>
				<div className=" py-3 px-5 rounded-md ">
					<ul className="flex flex-col">
						<li className="nav-item">
							<Link to='/home' className="nav-link flex justify-center py-5">
								<svg xmlns="http://www.w3.org/2000/svg" width="120" height="50" viewBox=".182 .548 511.304 171.879"
									id="instagram">
									<path
										fill="#000"
						...
						</li>
						{
							sidebar.map((item) => (
								<li key={item.id} className="nav-item">
									<Link to={item.href} className="nav-link flex items-center py-4 text-sm font-semibold px-4  hover:bg-gray-50">
										<span className="mx-4 font-bold">{item.icon}
										</span>
										{item.title}
									</Link>
								</li>
						...
						<Popconfirm
							title="Log Out"
							className="mt-20 flex items-center gap-2 font-semibold cursor-pointer"
							description="Are You Sure, You want to log out"
							onConfirm={success}
						>
							<LuLogOut size={20} />
